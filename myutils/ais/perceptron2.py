...
def report_xl(bl_ip):
    ...
    filename = TEST_DATE + '_report.xls'
    wb.save(filename)


def layer2(start_date,end_date): # SQL_BL에서 실제 BL을 추출하는 모델
    curs = conn.cursor()
    # bl
    train_ydata_s = 'select ip, bl_ibm, times, web_ok, web_rej, fw_pd, fw_db, waf_ok, waf_rej, ips_pd, ips_db, bl_dg_yn from aw_ip_cache where write_date >= %s and write_date < %s and (ip_gubun = 1 or ip_gubun = 2) and bl_ibm is not null and block_yn=1'
    curs.execute(train_ydata_s,(start_date,end_date))
    train_yrows = curs.fetchall()    
    train_ydf = pd.DataFrame(list(train_yrows), columns=['ip', 'bl_ibm', 'times', 'web_ok', 'web_rej', 'fw_pd', 'fw_db', 'waf_ok', 'waf_rej', 'ips_pd', 'ips_db',' bl_dg_yn'])       
    train_ydf['bl_dg_yn'] = 0
    ...
    # wl : bl에 비해 wl이 지나치게 많아서 수를 줄이기 위해 수집기간을 구분해서 저장
    curs = conn.cursor()
    train_xdata_s = 'select ip, bl_ibm, times, web_ok, web_rej, fw_pd, fw_db, waf_ok, waf_rej, ips_pd, ips_db, bl_dg_yn from aw_ip_cache where modi_time >= %s and modi_time < %s and (ip_gubun = 1 or ip_gubun = 2) and bl_ibm is not null and bl_dg_yn = 0 and block_yn=0'
    curs.execute(train_xdata_s,('2017-05-01',end_date))
    train_xrows = curs.fetchall()    
    train_xdf = pd.DataFrame(list(train_xrows), columns=['ip', 'bl_ibm', 'times', 'web_ok', 'web_rej', 'fw_pd', 'fw_db', 'waf_ok', 'waf_rej', 'ips_pd', 'ips_db',' bl_dg_yn'])       
    train_xdf['bl_dg_yn'] = 0
...
#def accuracy(TEST_DATE,ai_bl) :
    # n => normal
    # b => bl
    # bb => bl(real) -> bl(ai)
    # nb => normal(real) -> bl(ai)
    # bn => bl(real) -> normal(ai)
    # nn => normal(real) -> normal(ai)
...
ai_bl = layer2(LEARNING_START_DATE, LEARNING_END_DATE)
