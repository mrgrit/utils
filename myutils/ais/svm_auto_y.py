...
###############   CONST   #############
PCA_DIM = 10
LEARNING_START_DATE = '2017-05-01'
...
def data_preprocessing(start_date, end_date):
    curs = conn.cursor()
    # 학습데이터 - WL
    train_data_s = 'select ip, bl_ibm, times, web_ok, web_rej, fw_pd, fw_db, waf_ok, waf_rej, ips_pd, ips_db, bl_dg_yn from aw_ip_cache where modi_time >= %s and modi_time < %s and (ip_gubun = 1 or ip_gubun = 2) and bl_ibm is not null'
    ...
    # 예측할 오늘 데이터
    test_data_s = 'select ip, bl_ibm, times, web_ok, web_rej, fw_pd, fw_db, waf_ok, waf_rej, ips_pd, ips_db, bl_dg_yn from aw_ip_cache where modi_time >= %s and modi_time <= %s and (ip_gubun = 1 or ip_gubun = 2) and bl_ibm is not null'
    curs.execute(test_data_s,('2017-06-25',end_date,))
    test_rows = curs.fetchall()    
    test_df = pd.DataFrame(list(test_rows), columns=['ip', 'bl_ibm', 'times', 'web_ok', 'web_rej', 'fw_pd', 'fw_db', 'waf_ok', 'waf_rej', 'ips_pd', 'ips_db',' bl_yn'])
    test_df['bl_dg_yn'] = 0    
