...
def data_preprocessing(start_date, end_date):
    ...
    y_pred = do_nn(x_train, y_train, x_test, 10, 1e-5)
    ...
    ins1 = np.intersect1d(rf_res, dt_res)
    ins2 = np.intersect1d(ins1, svm_res)
    ins3 = np.intersect1d(ins2, nn_res)
...
def report_xl(bl_ip):
    ...
    ws2 = wb.create_sheet(title='SQL')    
    ws2.merge_cells('A1:L1')
    ws2['A1'] = str(today) + ' Threating IP Information From SQL'
    ca2 = ws2['A1']
    ...
    ws2['K2'] = 'IPS DB'
    ws2['L2'] = 'BL횟'
    ws2.freeze_panes = 'A3' # 셀 고정
    curs = conn.cursor()
    #times_today
    #select ip, cc, bl_ibm,
    #i = 0
    se_sql_bl = 'select ip from aw_ip_cache where bl_dg_yn = 1 and modi_time like %s'
    se_ip_ca = 'select ip,cc,bl_ibm,web_ok,fw_db,waf_ok,waf_rej,ips_pd,ips_db from aw_ip_cache where ip = %s'
    se_ti = 'select sum(times) from aw_log_full where source_ip = %s and write_date = %s'
    se_da = 'select count(distinct write_date) from aw_log_full where source_ip = %s'
    se_bl = 'select count(ip), modi_time from aw_bl where ip =%s group by ip' 

    curs.execute(se_sql_bl,(str(today) + '%',))
    sql_bl = curs.fetchall()
    print(len(sql_bl))
    
                           
    for i,ip in enumerate(sql_bl) :
        curs.execute(se_ip_ca,(ip,))
        bl_rows = curs.fetchall()        
        ...
        curs.execute(se_bl,(ip,))
        bl_ti = curs.fetchone()
        print('i => ',i)       
        ws2['A'+str(i+3)] = bl_rows[0][0]
        ws2['B'+str(i+3)] = bl_rows[0][1]
        ws2['C'+str(i+3)] = bl_rows[0][2]
        ws2['D'+str(i+3)] = ti[0]
        ws2['E'+str(i+3)] = da[0]
        ws2['F'+str(i+3)] = bl_rows[0][3]
        ws2['G'+str(i+3)] = bl_rows[0][4]
        ws2['H'+str(i+3)] = bl_rows[0][5]
        ws2['I'+str(i+3)] = bl_rows[0][6]
        ws2['J'+str(i+3)] = bl_rows[0][7]
        ws2['K'+str(i+3)] = bl_rows[0][8]
        ws2['L'+str(i+3)] = bl_ti[0]
...
bl_ip = data_preprocessing(LEARNING_START_DATE,LEARNING_END_DATE)
bl_ip = np.array(bl_ip.tolist())
report_xl(bl_ip)
