...
[7.Aug.17] 학습 데이터 8월 4일까지로 수정
[8.Aug.17] 학습 데이터 중 wl을 5월 1일 부터 오늘까지 중 order by modi_time desc 하여 최신 경향을 반영하도록 함(write_date가 맞는지 modi_time이 맞는지는 확인 해 봐야함)
[8.Aug.17] Slack Message 전송 기능 추가
[17.Aug.17] 완료시간 예측 모델, slack 전송 기능 추가
[30.Aug.17] 학습 데이터 8월 29일까지로 수정
...
LEARNING_END_DATE = '2017-08-29'
fw_db_times_limit = 30
web_ok_times_limit = 100
REPORT_TOP = 5
#######################################
...
def store_web_log_light(logdir,logfile,logday,result,conn) :
    ...
    for stri in ip_li:
        ip = stri.replace("Source IP: ","").replace("(Count=","").replace(")","")
        ip = ip.split(" ")
        if result == "web_ok" and int(ip[1]) < web_ok_times_limit :
            continue
        else :
...
def data_preprocessing(start_date, end_date):
    ...
    curs = conn.cursor()
    train_xdata_s = 'select ip, bl_ibm, times, web_ok, web_rej, fw_pd, fw_db, waf_ok, waf_rej, ips_pd, ips_db, bl_dg_yn from aw_ip_cache where modi_time >= %s and modi_time < %s and (ip_gubun = 1 or ip_gubun = 2) and bl_ibm is not null and bl_dg_yn = 0  order by modi_time desc limit 5000'
...
def report_xl(ai_bl):
    ...
    for idx,ip in enumerate(ai_bl) :
        ...
        da = curs.fetchone()
        print('i => ',idx)       
        ws['A'+str(idx+3)] = bl_rows[0][0]
        ws['B'+str(idx+3)] = bl_rows[0][1]
        ws['C'+str(idx+3)] = bl_rows[0][2]
        ws['D'+str(idx+3)] = ti[0]
        ws['E'+str(idx+3)] = da[0]
        ws['F'+str(idx+3)] = bl_rows[0][3]
        ws['G'+str(idx+3)] = bl_rows[0][4]
        ws['H'+str(idx+3)] = bl_rows[0][5]
        ws['I'+str(idx+3)] = bl_rows[0][6]
        ws['J'+str(idx+3)] = bl_rows[0][7]
        ws['K'+str(idx+3)] = bl_rows[0][8]
        ws['L'+str(idx+3)] = bl_rows[0][9]

        conn.commit()
        dic_tmp = {'ip':bl_rows[0][0],'cc':bl_rows[0][1],'ibm_score':bl_rows[0][2],
                    'times':ti[0],'day':da[0],'waf_ips':bl_rows[0][5]+bl_rows[0][6]+bl_rows[0][7]+bl_rows[0][8]}
        dic_data.append(dic_tmp)            

    filename = str(date.today()) + '_report.xls'
    wb.save(filename)


    ibm_top = sorted(dic_data, key=lambda k: k['ibm_score'],reverse=True)
    times_top = sorted(dic_data, key=lambda k: k['times'],reverse=True)
    times_min = sorted(dic_data, key=lambda k: k['waf_ips'],reverse=True)    
    day_top = sorted(dic_data, key=lambda k: k['day'],reverse=True)

    black_list = []

    slack_message('#general',"*<<IBM Score가 높은 IP 입니다.>>*")
    for i in range(REPORT_TOP) :
        message = str(ibm_top[i]['ip']+ ' : 국가 : '+ ibm_top[i]['cc']+ ', IBM Score : '+ str(ibm_top[i]['ibm_score'])+ ', 접속시도 횟수 : '+str(ibm_top[i]['times'])+', 지속 일수 : '+str(ibm_top[i]['day']))
        print(message)
        black_list.append(message)
        slack_message('#general', message)
    slack_message('#general',"*<<차단 횟수가 가장 많은 IP 입니다.>>*")
    for i in range(REPORT_TOP) :       
        message = str(times_top[i]['ip']+ ' : 국가 : '+ times_top[i]['cc']+ ', IBM Score : '+ str(times_top[i]['ibm_score'])+ ', 접속시도 횟수 : '+str(times_top[i]['times'])+', 지속 일수 : '+str(times_top[i]['day']))
        print(message)
        black_list.append(message)
        slack_message('#general', message)
    slack_message('#general',"*<<웹방화벽, IPS에 잡힌 IP입니다.>>*")
    for i in range(REPORT_TOP) :        
        message = str(times_min[i]['ip']+ ' : 국가 : '+ times_min[i]['cc']+ ', IBM Score : '+ str(times_min[i]['ibm_score'])+ ', 접속시도 횟수 : '+str(times_min[i]['times'])+', 지속 일수 : '+str(times_min[i]['day']))
        print(message)
        black_list.append(message)
        slack_message('#general', message)
    slack_message('#general',"*<<지속일수가 가장 많은 IP입니다.>>*")
    for i in range(REPORT_TOP) :        
        message = str(day_top[i]['ip']+ ' : 국가 : '+ day_top[i]['cc']+ ', IBM Score : '+ str(day_top[i]['ibm_score'])+ ', 접속시도 횟수 : '+str(day_top[i]['times'])+', 지속 일수 : '+str(day_top[i]['day']))
        print(message)
        black_list.append(message)
        slack_message('#general', message)

    lastm = "*사령관님!! *"+str(date.today()) + " AI가 색출 해 낸 블랙리스트 " +str(idx+1)+ " 개 입니다. *처단해 주십시오!!!*"
    slack_message("#general", lastm)
    message_set = set(black_list)
    for bll in message_set:
        slack_message('#general', bll)
        print(bll)

    curl = 'curl -F file=@'+filename+' -F channels=#general -F token=xoxp-223552584658-223479290179-225145163495-48232ac95a2792354c8382d3dc98a128 https://slack.com/api/files.upload'
    print(curl)
    os.system(curl)
...
def pred_end_time():
    ...
    print(int(pred/60)," 분 후에 완료될 예정입니다.")
    return int(pred/60)
...
leadTime = pred_end_time()
print(leadTime)
startMessage = '*사령관님 !! 데이터 입력이 모두 끝났습니다. 데이터 분석은 '+str(leadTime)+' 분이 소요될 예정입니다.*'
slack_message("#general",startMessage)
...
slack_message("#general", "사령관님!! 모든 작업이 완료되었습니다. AI가 잡아낸 블랙리스트들을 처단해 주세요")
