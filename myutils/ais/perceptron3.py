...
LEARNING_END_DATE = '2017-08-04'
TEST_DATE = '2017-08-11'
REPORT_TOP = 5
#######################################
...
def data_preprocessing(start_date, end_date):
    ...
    curs.execute(test_data_s,(TEST_DATE +'%',))
...
def report_xl(bl_ip):
    ...
    u_bl_etc = 'update aw_ip_cache set bl_etc = bl_etc+1 where ip = %s'
    ...
    for i,ip in enumerate(bl_ip) :
        ...
        curs.execute(se_ti,(ip, TEST_DATE))        
        ti = curs.fetchone()
        
        curs.execute(se_da,(ip,))
        da = curs.fetchone()

        curs.execute(u_bl_etc,(ip,))
    ...
    for i in range(REPORT_TOP) :        
        ...
        black_list.append(message)
        slack_message('#general', message)
    lastm = "사령관님!! "+str(date.today()) + " AI가 색출 해 낸 블랙리스트 " +str(i+1)+ " 개 입니다. 처단해 주십시오!!!"
...
report_xl(ai_bl)
write_log(TEST_DATE + ' end\n')
slack_message('#general','모든 프로세스가 종료되었습니다.')
